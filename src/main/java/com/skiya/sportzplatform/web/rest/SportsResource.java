...
public class SportsResource {
	...
	private final Logger log = LoggerFactory.getLogger(SportsResource.class);
	...
	public ResponseEntity<List<Sports>> getAllSports() {
		List<Sports> lookupList = sportsService.getAll();
		if (lookupList.isEmpty()) {
			return new ResponseEntity<List<Sports>>(HttpStatus.OK);
		}
		return new ResponseEntity<List<Sports>>(lookupList, HttpStatus.OK);
	}
	...
	public ResponseEntity<Sports> getSport(@PathVariable("sportsId") Integer sportsId) {
		...
		return Objects.nonNull(sports) && Objects.nonNull(sports.getSportsId())
				? new ResponseEntity<Sports>(sports, HttpStatus.OK)
				: new ResponseEntity<Sports>(HttpStatus.OK);
	}
	
	@RequestMapping(value = "/addSport", method = RequestMethod.POST)
	public ResponseEntity<?> createSport(@RequestBody Sports sport) {
