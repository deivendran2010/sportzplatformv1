...
import com.skiya.sportzplatform.service.CityService;
...
public class CityResource {
	...
	@Autowired
	private CityService stateService;
	
	
	
	@RequestMapping(value = "/all", method = RequestMethod.GET)
	public ResponseEntity<List<City>> getAllCompanies() {
		List<City> lookupList = stateService.getAll();
		if (Objects.isNull(lookupList)) {
			return new ResponseEntity<List<City>>(HttpStatus.OK);
		}
		return new ResponseEntity<List<City>>(lookupList, HttpStatus.OK);
	}
	
	@RequestMapping(value = "/{id}", method = RequestMethod.GET)
	public ResponseEntity<?> getCompany(@PathVariable("id") Integer id) {
		log.debug("Company Id..." + id);
		City state = stateService.getCompany(id);
		...
		return new ResponseEntity<City>(state, HttpStatus.OK);
	}
	@RequestMapping(value = "/addState", method = RequestMethod.POST)
	public ResponseEntity<?> createSport(@RequestBody City state) {
		log.debug("createState() << sport={}", state);
		City state1 = stateService.getCompanyByName(state.getCityName());
		if(Objects.nonNull(state1)) {
			log.error("Sports with same name exist.", state.getCityName());
			return new ResponseEntity<Object>(new RestResponse(false,"getCityName Name "+ state.getCityName() + " already exist!"),
					HttpStatus.OK);
		}
		...
		return Objects.nonNull(state.getStateId()) 
				? new ResponseEntity<City>(state, HttpStatus.CREATED)
				: new ResponseEntity<City>(HttpStatus.BAD_REQUEST);
