...
import com.skiya.sportzplatform.mapper.TournamentCategoriesMapper;

@Service
public class TournamentService extends AbstractBaseService{
	
	private final Logger log = LoggerFactory.getLogger(TournamentService.class);

	@Autowired
	TournamentCategoriesMapper sportsMapper;
	
	public TournamentCategories get(Integer sportsId) {
		TournamentCategories sport = sportsMapper.selectByPrimaryKey(sportsId);
		
		return sport;
	}
	
	/*public TournamentCategories getByName(String sportsName) {
		TournamentCategories sport = sportsMapper.selectBySportsName(sportsName);
	...
	public List<TournamentCategories> getAll() {
		List<TournamentCategories> sportList = sportsMapper.selectAll();
		/*if(Objects.nonNull(sportList)) {
			for(int a=0;a<sportList.size();a++) {
				TournamentCategories sport = sportList.get(a);
				if(Objects.nonNull(sport)) {
					sport.setSportsFormat(toJson(sport.getSportsFormat().toString(), Object.class));
					//sport.setSportsCategory(toJson(sport.getSportsCategory().toString(), Object.class));
				}
	...
	public int create(TournamentCategories sport) {
		...
		return sport.getTournamentCategoryId();
	}
	
	public int update(TournamentCategories sport) {
